...
class CommunityIndexer(Indexer):
    ...
    def index_community_downloads_recursive(self, community=None, time_period=None):
        if community is None or time_period is None:
            return
        ...
        self.logger.info("Indexing downloads for community: %s (%s)" %(community_id, community_name))
        self.index_community_downloads(community_id=community_id, time_period=time_period)

        if 'community' in community:
            sub_communities = community['community']
            for sub_community in sub_communities:
                self.index_community_downloads_recursive(community=sub_community, time_period=time_period)
        else:
            self.logger.info("There are no subcommunities in this community.")

    def index_community_collection_downloads(self, community_id=None, collection_id=None, time_period=None):
        if community_id is None or collection_id is None or time_period is None:
            return
